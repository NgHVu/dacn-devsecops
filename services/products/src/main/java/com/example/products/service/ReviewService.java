package com.example.products.service;

import com.example.products.dto.ReviewCreateRequest;
import com.example.products.dto.ReviewResponse;
import com.example.products.entity.Product;
import com.example.products.entity.Review;
import com.example.products.repository.ProductRepository;
import com.example.products.repository.ReviewRepository;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import org.springframework.http.HttpStatus;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;
import org.springframework.web.reactive.function.client.WebClient;
import org.springframework.web.server.ResponseStatusException;
import org.springframework.web.util.UriComponentsBuilder;

import java.net.URI;
import java.time.Duration;
import java.time.OffsetDateTime;

@Service
@RequiredArgsConstructor
@Transactional
@Slf4j
public class ReviewService {

    private static final String ERR_RATING_RANGE = "Đánh giá phải từ 1 đến 5 sao";
    private static final String ERR_PRODUCT_NOT_FOUND = "Không tìm thấy sản phẩm";
    private static final String ERR_REVIEW_NOT_FOUND = "Không tìm thấy đánh giá";
    private static final String ERR_FORBIDDEN = "Bạn không có quyền thao tác trên đánh giá này";
    private static final String ERR_DUPLICATE_REVIEW = "Bạn đã đánh giá sản phẩm cho đơn hàng này rồi.";
    private static final double RATING_ROUNDING_FACTOR = 10.0;

    private final ReviewRepository reviewRepository;
    private final ProductRepository productRepository;
    private final WebClient.Builder webClientBuilder;

    @Value("${app.services.orders.url:http://orders-app:8083}")
    private String ordersServiceUrl;

    public ReviewResponse createReview(String userId, ReviewCreateRequest req) {
        validateRating(req.rating());

        Product product = getProductOrThrow(req.productId());
        
        // Check điều kiện mua hàng
        verifyPurchaseCondition(userId, req.productId(), req.orderId());

        Review review = Review.builder()
                .userId(userId)
                .userName(req.userName() != null ? req.userName() : "Khách hàng ẩn danh")
                .product(product)
                .orderId(req.orderId())
                .rating(req.rating())
                .comment(req.comment())
                .build();

        review = reviewRepository.save(review);

        // Cập nhật rating sản phẩm: Thêm mới
        updateProductRatingStats(product, req.rating());

        return mapToResponse(review);
    }

    public ReviewResponse updateReview(Long reviewId, String userId, ReviewCreateRequest req) {
        Review review = getReviewOrThrow(reviewId);
        checkReviewOwnership(review, userId);
        validateRating(req.rating());

        int oldRating = review.getRating();

        review.setRating(req.rating());
        review.setComment(req.comment());
        review.setUpdatedAt(OffsetDateTime.now());

        review = reviewRepository.save(review);

        // Cập nhật rating sản phẩm: Sửa đổi
        recalculateProductRatingAfterUpdate(review.getProduct(), oldRating, req.rating());

        return mapToResponse(review);
    }

    public void deleteReview(Long reviewId, String userId, boolean isAdmin) {
        Review review = getReviewOrThrow(reviewId);

        if (!isAdmin) {
            checkReviewOwnership(review, userId);
        }

        Product product = review.getProduct();
        int ratingToRemove = review.getRating();

        reviewRepository.delete(review);

        // Cập nhật rating sản phẩm: Xóa
        recalculateProductRatingAfterDelete(product, ratingToRemove);
    }

    @Transactional(readOnly = true)
    public Page<ReviewResponse> getReviewsByProduct(Long productId, Pageable pageable) {
        if (!productRepository.existsById(productId)) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, ERR_PRODUCT_NOT_FOUND);
        }
        return reviewRepository.findByProductId(productId, pageable)
                .map(this::mapToResponse);
    }

    // --- Private Helper Methods (Clean Code) ---

    private void validateRating(int rating) {
        if (rating < 1 || rating > 5) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, ERR_RATING_RANGE);
        }
    }

    private Product getProductOrThrow(Long productId) {
        return productRepository.findById(productId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, ERR_PRODUCT_NOT_FOUND));
    }

    private Review getReviewOrThrow(Long reviewId) {
        return reviewRepository.findById(reviewId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, ERR_REVIEW_NOT_FOUND));
    }

    private void checkReviewOwnership(Review review, String userId) {
        if (!review.getUserId().equals(userId)) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, ERR_FORBIDDEN);
        }
    }

    private void verifyPurchaseCondition(String userId, Long productId, Long orderId) {
        if (reviewRepository.existsByUserIdAndProductIdAndOrderId(userId, productId, orderId)) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, ERR_DUPLICATE_REVIEW);
        }

        String orderStatus = fetchOrderStatus(userId, orderId);

        if (!"DELIVERED".equals(orderStatus) && !"COMPLETED".equals(orderStatus)) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "Đơn hàng chưa hoàn tất (Trạng thái: " + orderStatus + "). Vui lòng chờ giao hàng thành công.");
        }
    }

    private String fetchOrderStatus(String userId, Long orderId) {
        URI uri = UriComponentsBuilder
                .fromHttpUrl(ordersServiceUrl)
                .path("/api/internal/orders/{id}/status")
                .buildAndExpand(orderId)
                .toUri();

        log.debug("Checking order status at: {}", uri);

        try {
            return webClientBuilder.build()
                    .get()
                    .uri(uri)
                    .header("X-User-Id", userId)
                    .retrieve()
                    .bodyToMono(String.class)
                    .timeout(Duration.ofSeconds(5))
                    .block();
        } catch (Exception e) {
            log.error("Failed to verify order status. URL: {}", uri, e);
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Không thể xác thực trạng thái đơn hàng. Vui lòng thử lại sau.");
        }
    }

    // --- Rating Calculation Logic (Consolidated) ---

    private void updateProductRatingStats(Product product, Integer newRating) {
        long currentCount = product.getReviewCount();
        double currentAvg = product.getAverageRating();

        double newTotal = (currentAvg * currentCount) + newRating;
        updateProductStats(product, newTotal, currentCount + 1);
    }

    private void recalculateProductRatingAfterUpdate(Product product, int oldRating, int newRating) {
        long count = product.getReviewCount();
        if (count == 0) return;

        double currentTotal = product.getAverageRating() * count;
        double newTotal = currentTotal - oldRating + newRating;
        
        updateProductStats(product, newTotal, count);
    }

    private void recalculateProductRatingAfterDelete(Product product, int ratingToRemove) {
        long count = product.getReviewCount();
        if (count <= 1) {
            // Reset về 0 nếu xóa review cuối cùng
            product.setReviewCount(0);
            product.setAverageRating(0.0);
            productRepository.save(product);
        } else {
            double currentTotal = product.getAverageRating() * count;
            double newTotal = currentTotal - ratingToRemove;
            updateProductStats(product, newTotal, count - 1);
        }
    }

    private void updateProductStats(Product product, double newTotalScore, long newCount) {
        double newAvg = newTotalScore / newCount;
        // Làm tròn 1 chữ số thập phân
        newAvg = Math.round(newAvg * RATING_ROUNDING_FACTOR) / RATING_ROUNDING_FACTOR;

        product.setReviewCount((int) newCount);
        product.setAverageRating(newAvg);
        productRepository.save(product);
    }

    private ReviewResponse mapToResponse(Review review) {
        return new ReviewResponse(
                review.getId(),
                review.getUserId(),
                review.getUserName(),
                review.getOrderId(),
                review.getRating(),
                review.getComment(),
                review.getCreatedAt(),
                review.getUpdatedAt()
        );
    }
}